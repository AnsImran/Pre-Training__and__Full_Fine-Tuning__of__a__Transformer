from torch import ones, triu, bool, relu, sqrt, argmax, int32, float32, tensor
from torch.nn import Module, MultiheadAttention, BatchNorm1d, Linear, Dropout, Embedding, ModuleList
...
def positional_encoding(positions, d_model):
    ...
    return tensor(pos_encoding, dtype=float32)
...
def create_look_ahead_mask(matrix_of_sequences):
    ...
    #     mask = None
    # else:
    mask = triu(ones(matrix_of_sequences.shape[1], matrix_of_sequences.shape[1]), diagonal=1).to(bool)
    return mask
ones, triu, bool
...
class EncoderLayer(Module):
    def __init__(self, embedding_dim_, num_heads_, fully_connected_dim_,  dropout_rate_=0.1, layernorm_eps_=1e-6):
        ...
        self.attention           = MultiheadAttention(embed_dim=embedding_dim_, num_heads=num_heads_, dropout=dropout_rate_, batch_first=True)
        self.layernorm1          = BatchNorm1d(num_features=embedding_dim_, eps=layernorm_eps_)

        self.fc1                 = Linear(in_features=embedding_dim_, out_features=fully_connected_dim_)
        self.fc2                 = Linear(in_features=fully_connected_dim_, out_features=embedding_dim_)
        self.layernorm2          = BatchNorm1d(num_features=embedding_dim_, eps=layernorm_eps_)

        self.dropout = Dropout(dropout_rate_)
...
class Encoder(Module):
    ...
    def __init__(self, num_layers_, embedding_dim_, num_heads_, fully_connected_dim_, input_vocab_size_, maximum_position_encoding_, dropout_rate_=0.1, layernorm_eps_=1e-6):
        ...
        self.embedding    = Embedding(num_embeddings=input_vocab_size_, embedding_dim=self.embedding_dim, padding_idx=0)
        self.pos_encoding = positional_encoding(maximum_position_encoding_, self.embedding_dim)
        

        self.enc_layers = ModuleList([EncoderLayer(embedding_dim_=self.embedding_dim,
    ...
    def forward(self, current_batch_of_sequences):
        ...
        x = x * sqrt( tensor(self.embedding_dim, dtype=float32).to(x.device) )
...
class DecoderLayer(Module):
    ...
    def __init__(self, embedding_dim, num_heads, fully_connected_dim, dropout_rate=0.1, layernorm_eps=1e-6):
        ...
        self.mha1 = MultiheadAttention(embed_dim=embedding_dim, num_heads=num_heads, dropout=dropout_rate, batch_first=True)        
        self.mha2 = MultiheadAttention(embed_dim=embedding_dim, num_heads=num_heads, dropout=dropout_rate, batch_first=True) 

        self.fc1  = Linear(in_features=embedding_dim, out_features=fully_connected_dim)
        self.fc2  = Linear(in_features=fully_connected_dim, out_features=embedding_dim)

        self.layernorm1 = BatchNorm1d(num_features=embedding_dim, eps=layernorm_eps)
        self.layernorm2 = BatchNorm1d(num_features=embedding_dim, eps=layernorm_eps)
        self.layernorm3 = BatchNorm1d(num_features=embedding_dim, eps=layernorm_eps)
...
class Decoder(Module):
    ...
    def __init__(self, num_layers, embedding_dim, num_heads, fully_connected_dim, target_vocab_size,
        ...
        self.embedding = Embedding(num_embeddings=target_vocab_size, embedding_dim=self.embedding_dim, padding_idx=0)
        
        self.pos_encoding = positional_encoding(maximum_position_encoding, self.embedding_dim)

        self.dec_layers = ModuleList([DecoderLayer(embedding_dim=self.embedding_dim,
                                        num_heads=num_heads,
...
class Transformer(Module):
    ...
    def __init__(self, num_layers, embedding_dim, num_heads, fully_connected_dim, input_vocab_size, 
        ...
        self.final_layer = Linear(in_features=embedding_dim, out_features=target_vocab_size)
    ...
    def forward(self, input_sentence, output_sentence):
        ...
        final_output = log_softmax(final_output, dim=-1)
        ### END CODE HERE ###
...
def next_word(model, encoder_input, output):
    ...
    predicted_id = argmax(predictions, axis=-1).to(int32)
